...
2024/05/12 より、一般ユーザー向けのコマンドを「スラッシュコマンド」に対応させました。従来のテキストコマンドを実行しようとして構文を間違え、サーバーで恥ずかしい思いをする確率が低減されました。

メッセージ入力欄に `/` を入力すると候補が表示されますので、本Botのアイコンを選択後、実行したいコマンドを選択し、画面の内容に従って引数の指定などを行ってください。

基本的にDiscord側でスラッシュコマンドの説明は表示されますが、本ドキュメントはその補助資料として残しておきます。合わせてご活用ください。

ただし、デベロッパー専用コマンドなど、一部コマンドの実行はテキストコマンドとなります。接頭辞は「メンション」になります。
...
## ~~ChatGPT操作~~
### ~~OpenAIが開発するAIを使って会話ができます。暇つぶしなどにぜひ。~~ 【現在機能停止中】
#### コマンド：
##### gpt <引数>
コマンド名は「gpt」です。
